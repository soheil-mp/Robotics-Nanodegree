...
void process_image_callback(const sensor_msgs::Image img) {
    ...
    // 01) Loop through each pixel in the image and check if there's a bright white one
    ...
    // 02) Then, identify if this pixel falls in the left, mid, or right side of the image
    ...
    // 03) Depending on the white ball position, call the drive_bot function and pass velocities to it.
    // Request a stop when there's no white ball seen by the camera.
    if (where_to_move == 0){
